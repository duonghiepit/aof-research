...
def candle_stick_df(df,
                    ...
                    **kwargs):
    """
    Vẽ biểu đồ nến (candlestick) kèm theo các chỉ báo nâng cao từ DataFrame có cột:
        - 'Ngày' (chứa dữ liệu ngày tháng),
        - 'Giá (nghìn VNĐ)_Mở cửa', 'Giá (nghìn VNĐ)_Cao nhất',
          'Giá (nghìn VNĐ)_Thấp nhất', 'Giá (nghìn VNĐ)_Đóng cửa'
        - (nếu có) 'GD khớp lệnh_Khối lượng'

    show_advanced: danh sách các chỉ báo nâng cao muốn hiển thị:
        - 'volume': vẽ cột Volume bên dưới
        - 'macd': vẽ MACD line, Signal line, Histogram
        - 'rsi': vẽ chỉ báo RSI, kèm đường 70/30

    Returns:
        fig: Đối tượng Plotly Figure.
    """

    # 1. Sao chép DataFrame để tránh sửa trực tiếp
    df = df.copy()

    # 2. Đổi tên cột từ tiếng Việt sang chuẩn 'open', 'high', 'low', 'close', 'volume'
    rename_map = {
        'Ngày': 'Date',
        'Giá (nghìn VNĐ)_Mở cửa': 'open',
        'Giá (nghìn VNĐ)_Cao nhất': 'high',
        'Giá (nghìn VNĐ)_Thấp nhất': 'low',
        'Giá (nghìn VNĐ)_Đóng cửa': 'close',
        'GD khớp lệnh_Khối lượng': 'volume'
    }
    df.rename(columns=rename_map, inplace=True)

    # 3. Parse cột 'Date' sang datetime, set làm index
    if 'Date' in df.columns:
        # dayfirst=True giả định dữ liệu dạng DD/MM/YYYY
        df['Date'] = pd.to_datetime(df['Date'], dayfirst=True, errors='coerce')
        df.set_index('Date', inplace=True)

    # 4. Chuyển các cột giá và volume về dạng số (float)
    for col in ['open', 'high', 'low', 'close', 'volume']:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce')

    # 5. Kiểm tra các cột bắt buộc để vẽ candlestick
    required_cols = ['open', 'high', 'low', 'close']
    for col in required_cols:
        if col not in df.columns:
            raise ValueError(f"DataFrame phải chứa cột '{col}'!")
    
    # 6. Thiết lập tiêu đề mặc định (nếu chưa có)
    if title is None:
        title = "Stock Price & Advanced Metrics"

    # -------------------------------
    # CHIA SUBPLOT THEO CÁC CHỈ BÁO
    # -------------------------------
    ...
    if sorted(show_advanced) == ['macd', 'rsi', 'volume']:
        ...
        if sorted(show_advanced) == ['macd', 'rsi']:
            # Bạn có thể muốn chia 3 hàng, 2 hàng, tuỳ logic
            row_heights = [0.5, 0.3, 0.2]
            r_price, r_macd, r_rsi = 1, 2, 3
            r_volume = None
