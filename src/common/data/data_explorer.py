...
from src.explore.msn.const import _CURRENCY_ID_MAP, _GLOBAL_INDICES, _CRYPTO_ID_MAP
from src.core.utils.parser import get_asset_type
# from functools import wraps

logger = get_logger(__name__)
class StockComponents:
    """
    Class (lớp) quản lý các chức năng của thư viện liên quan đến cổ phiếu.
    """
...
class Quote:
    ...
    def __init__(self, symbol: str, source: str = "VCI"):
        ...
        if self.source not in self.SUPPORTED_SOURCES:
            raise ValueError(f"Hiện tại chỉ có nguồn dữ liệu từ {', '.join(self.SUPPORTED_SOURCES)} được hỗ trợ.")
        self.symbol = symbol.upper()
        self.source_module = f"src.explore.{source.lower()}"
...
class Listing:
    ...
    def __init__(self, source: str = "VCI"):
        ...
        if self.source not in self.SUPPORTED_SOURCES:
            raise ValueError(f"Hiện tại chỉ có nguồn dữ liệu từ {', '.join(self.SUPPORTED_SOURCES)} được hỗ trợ.")
        self.source_module = f"src.explore.{self.source.lower()}"
        self.data_source = self._load_data_source()
...
class MSNComponents:
    """
    Class (lớp) quản lý các chức năng của thư viện liên quan đến thị trường ngoại hối.
    """
